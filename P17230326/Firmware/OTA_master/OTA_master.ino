...
static unsigned long otaHold = 0;
...
void performOTA() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("No WiFi for OTA");
    return;
  }
...
void TaskNetwork(void *pvParameters) {
  ...
  while (1) {
    ...
    if (xQueueReceive(networkCmdQueue, &req, 100 / portTICK_PERIOD_MS)) {
      ensureWiFi();
  ...
  savePendingToFS();
      ...
      else if (req.cmd == NET_OTA_UPDATE) {
...
void TaskUI(void *pvParameters) {
  ...
  if (newMeas.temp < 35.0) {
    ...
    bool isOffline = (appState == STATE_OFFLINE || WiFi.status() != WL_CONNECTED);
...
if (isOffline) {
  ...
  if (millis() - otaHold > 3000) {
    otaTriggered = true;
    NetRequest req = {NET_OTA_UPDATE, "", 0, {}};
    xQueueSend(networkCmdQueue, &req, portMAX_DELAY);
  }
} else {
  otaTriggered = false;
...
Serial.println(WiFi.localIP());
  ...
  xTaskCreate(TaskWebServer, "Web_Task", 4096, NULL, 2, NULL);
}
